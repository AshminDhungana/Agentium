/**
 * preferences.ts — User Preference Service & API Client
 */
...
export interface PreferenceCategoryItem {
  id: string;
  name: string;
  description: string;
  icon: string;
...
export const PREFERENCE_CATEGORIES: readonly PreferenceCategoryItem[] = [
...
// ─── API Client (preferencesApi) ──────────────────────────────────────────────

export const preferencesApi = {
  async getAll(params?: { category?: string; scope?: string }): Promise<Preference[]> {
    const response = await api.get('/api/v1/preferences', { params: { category: params?.category, scope: params?.scope } });
