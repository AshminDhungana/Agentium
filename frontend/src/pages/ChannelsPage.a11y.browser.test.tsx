...
// Mock services used by ChannelsPage to avoid 500 errors in browser tests
vi.mock('@/services/api', () => ({
  api: vi.fn().mockImplementation(() => Promise.resolve({ data: [] })),
  rawFetch: vi.fn().mockResolvedValue({}),
}));

vi.mock('@/services/channelMetrics', () => ({
  channelMetricsApi: {
...
describe('ChannelsPage color-contrast', () => {
  ...
  it('passes in light theme', async () => {
    const result = await auditRoute(
      <MemoryRouter>{renderWithQueryClient(<ChannelsPage />)}</MemoryRouter>,
      'light'
    );
