...
class ExternalChannel(BaseEntity):
    ...
    # Credentials (encrypted at rest via application-layer encryption where configured)
...
class ChannelMetrics(BaseEntity):
    ...
    FIX: channel_id now has index=True so that the frequent
    filter_by(channel_id=...) queries (called on every poll cycle) use
    an index lookup instead of a full table scan.
    """

    __tablename__ = 'channel_metrics'

    # FIX #9 — index added; unique=True already implies an index on most
    # databases, but being explicit ensures Alembic generates it correctly
    # and makes the intent clear in code review.
    channel_id = Column(
        String(36),
        ForeignKey('external_channels.id'),
        nullable=False,
        unique=True,
        index=True,       # <-- ADDED: fast lookup for per-channel metric reads
    )
