...
class TestDelegationEngineIntegration:
    ...
    async def test_delegate_complex_task_to_tier2(self, seeded_db: Session):
        """Medium complexity task (4-6) should be delegated to LeadAgent (tier 2)."""
        head = seeded_db.query(HeadOfCouncil).filter_by(agentium_id="00001").first()
        ...
        ReincarnationService.spawn_lead_agent(
            ...
            name="Lead-Candidate",
            description="Lead candidate for medium-complexity routing",
            db=seeded_db,
        )
        seeded_db.commit()
        ...
        assert 4 <= result["complexity_score"] <= 6
    ...
    async def test_delegate_tier_mapping(self, seeded_db: Session):
        ...
        assert 1 <= result_low["complexity_score"] <= 3

        # Medium complexity (4-6) -> tier 2
        task_med = Task(
            agentium_id="TDEL007",
        ...
        seeded_db.add(task_med)
        seeded_db.flush()

        result_med = await DelegationEngine.delegate(task_med, seeded_db)
        assert result_med["delegation_metadata"]["target_tier"] == "2"
        assert 4 <= result_med["complexity_score"] <= 6

        # High complexity (7-10) -> tier 1 (Council)
        task_high = Task(
            agentium_id="TDEL008",
            title="High complexity",
            description="migrate refactor integrate architecture distributed security authentication",
            task_type=TaskType.ANALYSIS,
            status=TaskStatus.PENDING,
            priority=TaskPriority.CRITICAL,
            is_active=True,
        ...
        assert result_high["delegation_metadata"]["target_tier"] == "1"
        assert result_high["complexity_score"] >= 7

    @pytest.mark.asyncio
    async def test_delegate_complexity_7_10_to_tier1_council(self, seeded_db: Session):
        """Task complexity 7-10 should be delegated to Council (tier 1)."""
        head = seeded_db.query(HeadOfCouncil).filter_by(agentium_id="00001").first()
        council = seeded_db.query(CouncilMember).first()
        assert council is not None, "Council member must exist from seeding"

        task = Task(
            agentium_id="TDEL_COUNCIL_001",
            title="Security architecture review",
            description="migrate and refactor the distributed authentication architecture security",
            task_type=TaskType.ANALYSIS,
            status=TaskStatus.PENDING,
            priority=TaskPriority.CRITICAL,
            is_active=True,
    ...
    @pytest.mark.asyncio
    async def test_delegation_tracks_escalation_hops(self, seeded_db: Session):
        """Re-delegation with force=True should increment escalation_hop."""
        head = seeded_db.query(HeadOfCouncil).filter_by(agentium_id="00001").first()
        from backend.services.reincarnation_service import ReincarnationService
        ReincarnationService.spawn_task_agent(parent=head, name="Tier3-HOP", description="Test", db=seeded_db)
        seeded_db.commit()

        task = Task(
            agentium_id="TDEL_HOP_001",
            title="Test escalation hop",
            description="process data",
            task_type=TaskType.EXECUTION,
            status=TaskStatus.PENDING,
            priority=TaskPriority.NORMAL,
        ...
        # First delegation
        result1 = await DelegationEngine.delegate(task, seeded_db)
        assert "escalation_hop" in task.delegation_metadata
        assert task.delegation_metadata["escalation_hop"] == 0

        # Second delegation with force=True (simulates escalation)
        result2 = await DelegationEngine.delegate(task, seeded_db, force=True)
        assert task.delegation_metadata["escalation_hop"] == 1
        assert "escalation_history" in task.delegation_metadata
        assert len(task.delegation_metadata["escalation_history"]) == 1

    @pytest.mark.asyncio
    async def test_delegation_logs_escalation_audit(self, seeded_db: Session):
        """Escalation should create AuditLog entry with tier_escalation action."""
        ...
        # First delegation (to Council)
        await DelegationEngine.delegate(task, seeded_db)

        # Force re-delegation (simulates escalation)
        await DelegationEngine.delegate(task, seeded_db, force=True)

        # Check AuditLog for escalation entry
        audit = seeded_db.query(AuditLog).filter(
            AuditLog.action == "tier_escalation",
            AuditLog.target_id == task.id
        ).first()

        assert audit is not None
        assert audit.category == AuditCategory.GOVERNANCE
        assert audit.level == AuditLevel.WARNING
        
        import json
        meta = json.loads(audit.metadata_json) if audit.metadata_json else {}
        assert "from_tier" in meta
        assert "to_tier" in meta
        assert meta["escalation_hop"] == 1

    @pytest.mark.asyncio
    async def test_agent_registry_chooses_council_tier(self, seeded_db: Session):
        """AgentRegistry.choose_target should return Council member for tier '1'."""
        ...
        decision = Decision(
            ...
            target_tier="1",
            task_brief="Complex security audit",
            rationale="Requires Council deliberation"
        )

        target_id = await AgentRegistry.choose_target(decision, seeded_db, head)
    ...
    @pytest.mark.asyncio
    async def test_council_agents_exist_and_active(self, seeded_db: Session):
        """Verify Council members 10001, 10002 are seeded and active."""
        from backend.models.entities.agents import CouncilMember, AgentStatus

        council_1 = seeded_db.query(CouncilMember).filter_by(agentium_id="10001").first()
        council_2 = seeded_db.query(CouncilMember).filter_by(agentium_id="10002").first()

        assert council_1 is not None, "Council member 10001 not found"
        assert council_2 is not None, "Council member 10002 not found"

        for c in [council_1, council_2]:
            assert c.is_active is True
            assert c.status == AgentStatus.ACTIVE
