...
def test_4_1_2_security_startup_checks_strict(app, caplog, monkeypatch):
    ...
    monkeypatch.setenv("TESTING", "false")  # Ensure TESTING is false for security checks
...
def test_4_1_3_workspace_config_validation(app, caplog, monkeypatch):
    ...
    monkeypatch.setenv("AGENTIUM_WORKSPACE_ROOT", "/nonexistent")
...
def test_4_1_8_pricing_sync_background(app, caplog):
    ...
    caplog.set_level(logging.INFO, logger="backend.main")
    caplog.set_level(logging.INFO, logger="backend.services.pricing_sync_service")
    with TestClient(app) as client:
        response = client.get("/api/health")
        assert response.status_code == 200
        # Background pricing sync task is created
        assert "Background pricing synchronization" in caplog.text or "pricing cache" in caplog.text.lower() or "Loaded" in caplog.text
