...
- [x] **Host workspace persistence (sub-scope done)** — When a task finishes, generated artifacts are copied out of the Docker container into the host-visible directory `~/agentium-workspace/<agent_id>/<task_id>/` (via the `/host_home` bind mount). `file_tool`/`desktop_tool` route writes there, a `get_workspace` tool + system-prompt hint tell agents the path, the executor returns `workspace_path`+`artifacts`, and a `workspace_ready` WebSocket event surfaces the path to the user as a toast (tracked in `websocketStore.lastWorkspace`). Toggle with `AGENTIUM_WORKSPACE_ENABLED`; misconfig logged at startup. Branch: `feat/host-workspace-persistence`.
