...
case "$OS_FAMILY" in
    macos)
        HAS_MIC="true" ;;  # macOS handles mic permission at runtime
    linux|wsl2)
    ...
    windows)
        HAS_MIC="true" ;;  # PyAudio handles detection at runtime
esac
...
detect_svc_mgr() {
    case "$OS_FAMILY" in
        macos)   echo "launchd" ;;
        linux)
            # Check for a real user systemd session (not just the binary)
            if command -v systemctl &>/dev/null; then
                if systemctl --user status &>/dev/null 2>&1; then
                    echo "systemd"
                elif systemctl status &>/dev/null 2>&1; then
                    # System-level systemd exists but no user session (e.g. inside Docker)
                    # Try to start user session lingering
                    local uid
                    uid=$(id -u)
                    if command -v loginctl &>/dev/null && \
                       loginctl enable-linger "$uid" &>/dev/null 2>&1; then
                        sleep 1
                        systemctl --user status &>/dev/null 2>&1 && echo "systemd" || echo "none"
                    else
                        echo "none"
                    fi
                else
